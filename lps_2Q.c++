...
void merge(vector<point> &elements, int left, int mid, int right)
{
	...
	while( (index_Left_SA<no_Left_Sub) && (index_Right_SA<no_Right_Sub))
	{
		...
		if(left_Sub_Array[index_Left_SA].x < right_Sub_Array[index_Right_SA].x)
		{
			// copy the element from left sub array into orginal array and increment indices
			elements[index_Full_Array].x = left_Sub_Array[index_Left_SA].x;
			elements[index_Full_Array].y = left_Sub_Array[index_Left_SA].y;
			index_Left_SA++;
			index_Full_Array++;
		}
		//otherwise
		else
		{
			...
			elements[index_Full_Array].x = right_Sub_Array[index_Right_SA].x;
			elements[index_Full_Array].y = right_Sub_Array[index_Right_SA].y;
			index_Right_SA++;
			index_Full_Array++;
		}
	}
